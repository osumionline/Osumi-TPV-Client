...
export type PurchaseOrderTotalsEditableField = 'portes' | 'descuento';

/**
 * Limita los decimales introducidos sin impedir
 * estados intermedios de edición.
 */
export function limitPurchaseOrderTotalsDecimalFraction(
  value: string,
...
/**
 * Indica si el texto representa un estado decimal
 * temporal todavía incompleto.
 */
export function isPurchaseOrderTotalsTransientDecimal(value: string): boolean {
...
/**
 * Convierte un decimal con coma o punto al entero
 * escalado utilizado por el modelo económico.
 */
export function parsePurchaseOrderTotalsDecimal(value: string, scale: number): number | null {
  const trimmedValue: string = value.trim();
...
/**
 * Formatea un entero escalado utilizando coma decimal.
 */
export function formatPurchaseOrderTotalsDecimal(
  value: number,
  scale: number,
...
/**
 * Obtiene el total que debe presentarse en la ficha.
 * Un recepcionado conserva siempre su importe histórico.
 */
export function getPurchaseOrderDisplayedTotalMicros(
  recepcionado: boolean,
  importeMicros: number,
  totals: PurchaseOrderTotals,
): number {
  return recepcionado ? importeMicros : totals.totalFacturaMicros;
}
